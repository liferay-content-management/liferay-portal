...
public class ForumNotificationRestController extends BaseRestController {
	...
	private String _fetchMessageTitle(long threadId, String authToken) {
		try {
			String response = _liferayApiClient.get(
				"/o/c/c2m0forumthreads/" + threadId + "?fields=messageTitle",
				authToken);
	...
	private void _processNewReply(String json, String authToken) {
		...
		if (valuesJSONObject != null) {
			threadId = valuesJSONObject.optLong(_THREAD_ID_FIELD_NAME, 0L);
		...
		if (threadId == 0L) {
			if (_log.isWarnEnabled()) {
				_log.warn(
					"onNewReply: missing " + _THREAD_ID_FIELD_NAME +
						" in payload");
			}
	...
	private void _processUpdatedReply(String json, String authToken) {
		...
		if (threadId == 0L) {
			if (_log.isWarnEnabled()) {
				_log.warn(
					"onUpdatedReply: missing " + _THREAD_ID_FIELD_NAME +
						" in payload");
			}
	...
	private static final String _THREAD_ID_FIELD_NAME =
		"r_c2m0FrmThrToC2M0FrmMsgs_c_c2m0ForumThreadId";
