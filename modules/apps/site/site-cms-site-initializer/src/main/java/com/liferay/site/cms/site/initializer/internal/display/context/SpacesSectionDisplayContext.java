...
public class SpacesSectionDisplayContext {
	...
	private Collection<AssetLibrary> _getAssetLibraries(
		...
		if (assetLibrariesPinnedByMePage.getTotalCount() == 5) {
			return assetLibrariesPinnedByMePage.getItems();
		}

		List<AssetLibrary> assetLibrariesPinnedByMe = new ArrayList<>(
			assetLibrariesPinnedByMePage.getItems());

		List<Long> assetLibraryPinnedByMeIds = ListUtil.toList(
			assetLibrariesPinnedByMe, assetLibrary -> assetLibrary.getId());

		for (AssetLibrary assetLibrary : assetLibrariesPage.getItems()) {
			if (!assetLibraryPinnedByMeIds.contains(assetLibrary.getId())) {
				assetLibrariesPinnedByMe.add(assetLibrary);
			}

			if (assetLibrariesPinnedByMe.size() == 5) {
				return assetLibrariesPinnedByMe;
			}
		}
