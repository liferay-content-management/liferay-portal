...
public class DLAppServiceWhenCopyingAFolderTest extends BaseDLAppTestCase {
	...
	public void testShouldSucceedBetweenDifferentSites() throws Exception {
		...
		Map<String, List<String>> foldersAndFilesNames =
			_getRandomFoldersAndFilesNames(3);

		_createFoldersAndFiles(foldersAndFilesNames, serviceContext);
		...
		AssertUtils.assertEquals(
			foldersAndFilesNames, _getFoldersAndFilesNames(folder));
	}

	private void _createFoldersAndFiles(
			Map<String, List<String>> foldersAndFilesNames,
			ServiceContext serviceContext)
		throws Exception {

		for (Map.Entry<String, List<String>> entry :
				foldersAndFilesNames.entrySet()) {
	...
	private Map<String, List<String>> _getFoldersAndFilesNames(
			Folder parentFolder)
		throws Exception {

		Map<String, List<String>> foldersAndFilesNames = new HashMap<>();
		...
		for (Folder folder : folders) {
			...
			foldersAndFilesNames.put(folder.getName(), fileNames);
		}

		return foldersAndFilesNames;
	}

	private Map<String, List<String>> _getRandomFoldersAndFilesNames(
		int numberOfFolders) {
		...
		for (int i = 0; i < numberOfFolders; i++) {
			...
			Collections.sort(fileNames);
			foldersAndFilesNames.put(folderNames.get(i), fileNames);
		}
