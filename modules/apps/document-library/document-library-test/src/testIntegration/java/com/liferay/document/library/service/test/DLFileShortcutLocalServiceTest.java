...
public class DLFileShortcutLocalServiceTest {
	...
	public void testAddFileShortcut() throws Exception {
		...
		Assert.assertEquals(
			fileShortcut.getToFileEntryId(), dlFileEntry.getFileEntryId());
	...
	public void testAddFileShortcutWithFolder() throws Exception {
		...
		Assert.assertEquals(
			dlFileEntry.getFileEntryId(), fileShortcut.getToFileEntryId());

		Assert.assertEquals(
			dlFileEntry.getFolderId(), fileShortcut.getFolderId());
	}
	...
	public void testAddFileShortcutWithInvalidFileEntry() throws Exception {
		DLAppLocalServiceUtil.addFileShortcut(
			TestPropsValues.getUserId(), _group.getGroupId(),
			DLFolderConstants.DEFAULT_PARENT_FOLDER_ID, -1, _serviceContext);
	}
	...
	public void testAddFileShortcutWithInvalidFolder() throws Exception {
		...
		FileShortcut fileShortcut = DLAppLocalServiceUtil.addFileShortcut(
			TestPropsValues.getUserId(), _group.getGroupId(), -1,
			dlFileEntry.getFileEntryId(), _serviceContext);
