...
export function DrawSurface({
		...
		onAnnounce(
			Liferay.Language.get(
				guided
					...
					: 'the-drawing-has-started.-click-or-press-enter-to-add-points-and-finish-on-the-last-point'
			)
		);
	}, [guided, onAnnounce]);
	...
	const cancel = () => {
		...
		onAnnounce(Liferay.Language.get('the-drawing-was-canceled'));
	};
		...
		switch (event.key) {
			...
			case 'Enter':
				if (guide.stage === 'end') {
					...
					onAnnounce(
						Liferay.Language.get(
							'the-line-is-set.-move-the-middle-point-with-the-arrow-keys-to-curve-the-line-then-press-enter-to-finish'
						)
					);
	...
	const handleKeyDown = (event: React.KeyboardEvent<SVGRectElement>) => {
		...
		switch (event.key) {
			...
			case 'Delete':
				...
				onAnnounce(Liferay.Language.get('the-point-was-removed'));
				break;
