...
public class KBArticleLocalServiceTest {
	...
	@FeatureFlags("LPS-195016")
	@Test
	public void testDeleteKBArticleWithLockByOtherUser() throws Exception {
		KBArticle kbArticle = _addKbArticle();

		_withLockByOtherUser(
			kbArticle.getResourcePrimKey(),
			() -> _kbArticleLocalService.deleteKBArticle(kbArticle));
	}
	...
	@FeatureFlags("LPS-195016")
	@Test
	public void testExpireKBArticleWithLockByOtherUser() throws Exception {
		KBArticle kbArticle = _addKbArticle();
	...
	@FeatureFlags("LPS-195016")
	@Test
	public void testMoveKBArticleToTrashKBArticleWithLockByOtherUser()
		throws Exception {
	...
	@FeatureFlags("LPS-195016")
	@Test
	public void testMoveKBArticleWithLockByOtherUser() throws Exception {
		KBArticle kbArticle = _addKbArticle();
		KBArticle parentKBArticle = _addKbArticle();
	...
	@FeatureFlags("LPS-195016")
	@Test
	public void testRevertKBArticleWithLockByOtherUser() throws Exception {
		KBArticle kbArticle = _addKbArticle();
	...
	@FeatureFlags("LPS-195016")
	@Test
	public void testUpdateKBArticleWithLockByOtherUser() throws Exception {
		KBArticle kbArticle = _addKbArticle();
	...
	private void _withLock(
			long resourcePrimKey,
			UnsafeRunnable<PortalException> unsafeRunnable)
		throws PortalException {

		_kbArticleLocalService.lock(_user.getUserId(), resourcePrimKey);

		Assert.assertTrue(
			LockManagerUtil.hasLock(
				_user.getUserId(), KBArticleConstants.getClassName(),
				resourcePrimKey));
	...
	private void _withLockByOtherUser(
			long resourcePrimKey,
			UnsafeRunnable<PortalException> unsafeRunnable)
		throws Exception {
		...
		try {
			Lock lock = _kbArticleLocalService.lock(
				otherUser.getUserId(), resourcePrimKey);

			Assert.assertTrue(
				LockManagerUtil.hasLock(
					otherUser.getUserId(), KBArticleConstants.getClassName(),
					resourcePrimKey));
			...
			try {
				...
				Assert.assertEquals(
					duplicateLock.getLockId(), lock.getLockId());
			}

			_kbArticleLocalService.unlock(
				otherUser.getUserId(), resourcePrimKey);
			...
			_userLocalService.deleteUser(otherUser);
		}
	}
