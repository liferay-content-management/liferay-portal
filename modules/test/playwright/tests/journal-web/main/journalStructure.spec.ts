...
test(
	...
	async ({apiHelpers, journalStructuresPage, page, site}) => {
		...
		expect(
			alertBoundingBox.y,
			'The alert must start below the sticky toolbar instead of being clipped behind it'
		...
		await expect
			.poll(async () => (await alert.boundingBox()).y, {
				message:
					'The page must scroll so that the toolbar is kept in place by its own sticky positioning',
			})
		...
		expect(
			...
			'The toolbar must stay pinned below the control menu while the page scrolls'
